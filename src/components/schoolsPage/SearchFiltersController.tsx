...
export const searchFiltersControllerConfig = createSearchControllerConfig('filters', {
  defaultValue: Object.fromEntries(
      Object.values(filters).map(f => [f[idKey], []]),
  ),
  toParamHandler: ({value, key, mode, p }) => {
      Object.entries(value as Record<string, string[]>).forEach(([filterKey, filterValues]) => {
        const filter = filters.find(f => f[idKey] === filterKey);
        if (filterValues.length > 0 && filter) {
          p.set(mode === 'search' ? filter[idKey] : filter.apiParam, transformArr(filterValues, mode));
  ...
  fromParamHandler: ({p, key}) => {
    ...
    filters.forEach((filter: FilterData) => {
      ...
      const param = p.has(filterKey) && p.get(filterKey) ? p.get(filterKey) : null;
      if (param) {
