...
const SearchOrderingController: FC = () => {
  ...
  const [active, setActive] = useState(
    getSearchViewById(searchData.view)?.layout?.enableOrdering,
  );
  ...
  useEffect(() => {
    if (searchData.ordering !== ordering) setOrdering(searchData.ordering);
  }, [searchData.ordering]);
  ...
  const changeOrdering = (newOrdering: string) =>
    setOrdering(newOrdering === ordering ? null : newOrdering);
...
export const searchOrderingControllerConfig = createSearchControllerConfig(
  'ordering',
  {
    defaultValue: null,
    toParamHandler: ({ value, key, mode, p }) => {
      ...
      if (mode === 'api') p.set(key, orderingType.fields.join(','));
      else p.set(key, orderingType.orderingId);
    },
