...
const MapSearchView: FC<SearchViewProps> = ({isFetching, schools, count, fetchedAll}) => {
    ...
    useEffect(() => {
        ...
        schools.map(school => {
            if(!school.address || !school.address.longitude || !school.address.latitude)
                return;
            ...
            markers.current.push(
                L.marker(coords)
                    .addTo(map.current)
                    .bindPopup(`<a href="/school/${school.id}">${school.school_name}</a>`)
            );
        })
    }, [schools]);
